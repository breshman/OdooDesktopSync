...
class IoTManager {
  ...
  void notifyStateChanged() {
    // Notify Riverpod of the state change
    ref.read(iotStateProvider.notifier).state = IoTState(
  ...
  static void registerDriver({
    ...
    driverRegistry.add(DriverRegistryEntry(
      factory: factory,
  ...
  DriverRegistryEntry? findCompatibleDriver(String connectionType, dynamic device) {
  ...
  void triggerDeviceEvent(String deviceIdentifier, Map<String, dynamic> response) {
    final event = {
    ...
    final pendingToResolve = List<Completer<Map<String, dynamic>>>.from(_pendingEventRequests);
  ...
  Future<Map<String, dynamic>?> waitForEvent(Map<String, dynamic> listener) async {
    final devices = List<String>.from(listener['devices'] ?? []);
    final double lastEventTime = (listener['last_event'] as num?)?.toDouble() ?? 0.0;

    // Check if we already have a newer event matching criteria
    for (final event in events) {
      if (devices.contains(event['device_identifier']) && (event['time'] as double) > lastEventTime) {
        return event;
      }
    }
  ...
  Future<void> startAll() async {
    loggingService.info('Starting IoT Manager with ${interfaces.length} active interfaces.');
...
// State provider exposed to UI components
final iotStateProvider = StateProvider<IoTState>((ref) {
